# 📈 FINAL STOCK TREND APP WITH WATCHLIST REMOVED FOR SERVICE PROVIDERS ONLY
...
if not st.session_state.logged_in:
    ...
    if st.session_state.user_role == "remote_users":
        ...
        if watchlist:
            ...
            st.success(f"📈 Uptrend: {uptrend}" if uptrend else "📈 No stocks in uptrend")
            st.error(f"📉 Downtrend: {downtrend}" if downtrend else "📉 No stocks in downtrend")

    # 🔍 Search & Predict
    st.subheader("🔍 Search and Predict a Stock")
    ...
    if st.button("Predict Trend") and search_symbol:
        try:
            ...
            (X_train, X_test, y_train, y_test), _ = preprocess(df)
            prediction = get_final_prediction(X_train, X_test, y_train, y_test)
            st.success("📈 Predicted Uptrend" if prediction == 1 else "📉 Predicted Downtrend")
